...
<div class="flex w-full mt-4 place-content-between">
    <form action="{{ route('customer.home') }}" method="GET" class="flex items-center gap-2">
        <h3 class="text-lg py-2">Unit Produksi: </h3>
        <select id="vocationId" name="vocationId" class="block rounded-lg text-lg border-gray-300 focus:outline-none px-4 py-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200" onchange="this.form.submit()">
    ...
    <form action="{{ route('customer.home') }}" method="GET" class="flex items-center gap-2">
        @csrf
        <input type="text" name="search" value="{{ request()->query('search') }}" placeholder="Cari produk" class="border border-gray-300 rounded-lg px-2 py-2">
...
<div class="w-full grid grid-cols-6 gap-4 mt-4">
    @foreach ($products as $product)
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }} Image" class="w-full aspect-square object-cover">
        <div class="p-4">
            <h3 class="text-md font-normal text-ellipsis">{{ $product->name }}</h3>
            <p class="text-sm text-gray-600 text-ellipsis">{{ $product->vocational->name }}</p>
            <!-- <div class="flex justify-between items-center mt-4"> -->
                <h3 class="font-bold text-xl mt-2">Rp {{ number_format($product->price, 0, ',', '.') }}</h3>
